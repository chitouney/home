...
# The prompt
NEWLINE=$'\n'
PROMPT='%{$fg_bold[yellow]%}☢%{$reset_color%} %{$fg_bold[cyan]%}[%~] %{$reset_color%}${NEWLINE}%{$fg_bold[white]%}☯ %{$reset_color%}%{$fg_bold[green]%} ➟ %{$reset_color%} '
...
# Compute and set title of the xterm
...
function currentDir()
{
    local currentPath=$(pwd | awk -F/ -v "n=$(tput cols)" -v "h=^$HOME" '{sub(h,"~");n=0.3*n;b=$1"/"$2} length($0)<=n || NF==3 {print;next;} NF>3{b=b"/../"; e=$NF; n-=length(b $NF); for (i=NF-1;i>3 && n>length(e)+1;i--) e=$i"/"e;} {print b e;}')
    printf $currentPath
}
...
case $TERM in
    xterm*)
        ...
        precmd () {print -Pn "\e]0;☣ $currentPath\a"}
        ;;
esac
